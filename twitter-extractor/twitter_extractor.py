## We used this script to download Twitter data. It authenticates at Twitter through application-only authentication [1].
...
class TwitterReader:
    ...
    # Private class definition and members
    ...
    _limits = {   # A dictionary that contain resources for rate limits information. The key represents a resource that contains two associated keys: remaining = how many requests are left for the resource; renew_epoch = next epoch to renew the window for the resource.
               '/users/show'            : {
                                           'remaining'      : None,
                                           'renew_epoch'    : None,
                                          },
    ...
    def _get_request_headers(self):
        ...
        data = response.read().decode('utf-8')  # please check note on https://docs.python.org/2/library/httplib.html#httplib.HTTPConnection.getresponse
    ...
    def _check_limit_remaining(self, resource):
        if self._limits[resource]['remaining'] <= 0:
            if self._limits[resource]['remaining'] == -1:       # Twitter will not send the rate limits headers, request these limits
    ...
    # Class definition that is public
    ...
    def get_friendship(self, source_screen_name, target_screen_name):
        ...
        response = self._connection.getresponse()
        data = response.read().decode('utf-8')  # Splease check note on https://docs.python.org/2/library/httplib.html#httplib.HTTPConnection.getresponse
        self._handle_twitter_response_code(response, data)
